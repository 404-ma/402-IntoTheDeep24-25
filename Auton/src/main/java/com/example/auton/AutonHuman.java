...
public class AutonHuman implements Auton {
    public void Run(Runner runner, IGrabber grabber) {
        ...
        grabber.HangSample();
        while (grabber.CheckForBrake()) ;
        runner.move(b -> b.waitSeconds(0.2).lineToYConstantHeading(-40));
        ...
        while (grabber.CheckForBrake()) ;
        runner.move(b -> b.waitSeconds(0.1).lineToYConstantHeading(-58));
        grabber.Close();
        runner.move(b -> b.waitSeconds(0.2));
        grabber.GoToLowBar();
        while (grabber.CheckForBrake()) ;
        runner.move(b -> b.lineToYConstantHeading(-40));
        grabber.GoToHighBar();
        runner.move(b -> b.splineTo(getStartingPose().position, getStartingPose().heading));
        while (grabber.CheckForBrake()) ;
        runner.move(b -> b.strafeToConstantHeading(new Vector2d(4, -34)));
        grabber.HangSample();
        while (grabber.CheckForBrake()) ;
        runner.move(b -> b.waitSeconds(0.2).lineToYConstantHeading(-40));
        grabber.GoToPickupHeight();
        ...
        runner.move(b -> b.splineTo(getStartingPose().position, getStartingPose().heading));
        while (grabber.CheckForBrake()) ;
        runner.move(b -> b.strafeToConstantHeading(new Vector2d(2, -34)));
        grabber.HangSample();
        while (grabber.CheckForBrake()) ;
